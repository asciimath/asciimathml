...
export class AsciiMathParser {
   ...
   * @type {TokenType}
   */
  private previousSymbol: TokenType = TokenType.NONE;
   ...
   * @type {TokenType}
   */
  private currentSymbol: TokenType = TokenType.NONE;
  ...
  private getSymbol(str: string): Symbol | null {
    ...
    if (
      ...
      return { input: st, tag: tagst, output: st=="-"?"\u2212":st, tex: null, ttype: TokenType.UNARY, func: true };
    }

    return { input: st, tag: tagst, output: st=="-"?"\u2212":st, tex: null, ttype: TokenType.CONST };
  }
   ...
   */
  private appendUnwrap(toappend: INodeAdapter, node: INodeAdapter) {
  ...
  private parseSexpr(str: string): ParseResult {
    ...
    symbol = symbol as Symbol; // checked it's not null
  ...
  */
   ...
   */
  public mml(_string: string): INodeAdapter | null {
    this.nestingDepth = 0;
