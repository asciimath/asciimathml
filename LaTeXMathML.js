...
(function() {
  ...
  classid=\"clsid:32F66A20-7614-11D4-BD11-00104BD3F987\"></object>");
    ...
    function parseMath(str, latex) {
		...
		var arr1 = [], arr2 = [], arr3 = [], startPos = [], endPos = [], before = [], after = [];
		do{
			found = false;
			p1 = str.indexOf('\\begin{array}{ccc}', p2);
			if(p1 > -1)
			{
				p2 = str.indexOf('\\end{array}', p1+15);
				if(p2 > -1)
				{
					arr1.push(str.substr(p1, p2+11-p1));
					arr2.push(str.substr(p1+18, p2-p1-18));
					var sp, ep;
					sp = p1;
					ep = p2+11;
					startPos.push(sp);
		...
		while(found);
		var i, j, k, l, m, n, o, q, r, s, objectArray = {};
		for(i in arr2)
		{ 
			j = arr2[i];
			k = j.trim();
			...
			//
			objectArray.matrix = r;
			objectArray.startPos = startPos[i];
			objectArray.endPos = endPos[i];
			objectArray.before = before[i];
			objectArray.after = after[i];
			
			arr3.push(objectArray); // matrix
		}
		if(arr3.length)
		{
			for(i in arr3)
	...
	function latexToMathML(latexString)
	{
		var elem = parseMath(latexString, true);
		elem.setAttribute('xmlns', AMmathml);
		return elem.outerHTML;
	}
	function latexToSVG(latexString)
	{
		var elem = parseMath(latexString, true);
		elem.setAttribute('xmlns', AMmathml);
